...
//openocd -f ~/Data/NetbeansProjects/STM32/F103/STM32F1_I2C_TP_conf_writer/STM32F103C8T6.cfg

#include "stm32f103_db.h"
#include "f103_i2c_tp_conf_writer.h"
#include "config.h"
#include "usb.h"
...
int main(void)
{
  ...
  InitIOPin(GPIOB, 10, GPIO_MODE_2MHZ, GPIO_CONF_AF_OD);
  
  //Input to monitor SDA signal
  InitIOPin(GPIOB, 11, GPIO_MODE_2MHZ, GPIO_CONF_AF_OD);
  ...
  //Start with slave address 0x14
  int addr = 0x14;

  //For writing the TP conf the following is needed
  //Start
  //Output 0x28 (write to slave 0x14 0x 0x5D)
  //Output 0x80
  //Output 0x47
  //Write 186 bytes
  //Stop

  //Address slave for writing the register address and data
  ...
  {
    //If no response on 0x14 try 0x5D
    addr = 0x5D;
...
int writei2cconfig(void)
{
  ...
  //Write all the config bytes
  for(i=0;i<sizeof(config_data);i++)
  {
    //Send first address byte
    if(sendi2cbyte(config_data[i]) == 0)
    {
      return(0);
    }
